...
sys.path.insert(0, "/gpfs/home4/asiddarth/ViTST/code")
...
def fine_tune_hf(
    ...
    def preprocess_train(example_batch):
        """Apply train transforms on already loaded PIL images."""
    ...
    def preprocess_val(example_batch):
        """Apply val transforms on already loaded PIL images."""
        example_batch["pixel_values"] = [
    ...
    # Replace empty val_dataset with test_dataset
    if isinstance(val_dataset, list) or len(val_dataset) == 0:
        print("No validation dataset found. Using test dataset as validation instead.")
        val_dataset = test_dataset
    
    # Only call set_transform if val_dataset supports it
    if hasattr(val_dataset, "set_transform"):
        val_dataset.set_transform(preprocess_val)
    else:
        print("Warning: val_dataset is not a HuggingFace Dataset. Skipping set_transform.")
...
if __name__ == "__main__":
    ...
    if dataset == 'P12':
        ...
        upsample = False  # not needed unless classes are imbalanced
        epochs = 50  # adjust for your use case
        image_size = (384,384)  # match how you created images
        grid_layout = (6,6)
    ...
    for missing_ratio in missing_ratios:
        ...
        # for k in range(4,5):
            ...
            flattened_test_labels = [int(y[0]) if isinstance(y, (list, np.ndarray)) else int(y) for y in ytest]
            if len(set(flattened_test_labels)) < 2:
                print("Skipping split due to single-class test label - AUROC/F1 may be invalid.")
